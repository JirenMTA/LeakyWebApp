...
async def notif_create_order(user_id: int, order: Orders):
    text_message = (f'Вы успешно создали заказ <b>№{order.id}</b>!<p>Сумма к оплате: <b>{order.total_price}</b></p>'
                    f'<p><a href="127.0.0.1:3000/orders"/> </p>')
    try:
...
async def notif_create_promo(promo: Promo):
    text_message = (f'<b>Добавлен новый промокод</b>!<p>{promo.sale}: {promo.code}</p>'
                    f'<p><b>Успейти им воспользоваться!</b></p>')
    try:
